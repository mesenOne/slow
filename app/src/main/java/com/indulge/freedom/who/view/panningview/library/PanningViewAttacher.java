package com.indulge.freedom.who.view.panningview.library;
...
public class PanningViewAttacher implements ViewTreeObserver.OnGlobalLayoutListener {
	...
	private boolean mIsPortrait2 = false;
...
//	private boolean mIsPortrait2 = true;
...
//		mImageView = null;
	...
	private void animate_() {
		...
		if(mIsPortrait) {
			if(mWay == Way.R2L) {
				if (mDisplayRect.left<10){
					animate(mDisplayRect.left, mDisplayRect.left - (mDisplayRect.right - getImageViewWidth()),200000);
				}
 				else{
				...
				if (mIsPortrait2){
					...
					if(mIsPortrait) {
						if(value<-10){
							if(times==0){
								stopPanning();
								animate(mDisplayRect.left, mDisplayRect.left - (mDisplayRect.right - getImageViewWidth()), 100000);
								times++;
								return;
							}
						}
